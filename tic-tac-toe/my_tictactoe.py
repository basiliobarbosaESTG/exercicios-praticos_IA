...
NUM_ROWS = LENGTH
NUM_COLS = LENGTH
# Num states can be x,o or empty for each position in the board
NUM_STATES = 3**(NUM_ROWS*NUM_COLS)


class Environment:
    def __init__(self, length_r=NUM_ROWS, length_c=NUM_COLS, print_board=False):
        # Game board, init as empty
        self.board = np.zeros((length_r, length_c))
        self.x = -1  # player 1
        self.o = 1  # player 2
        self.winner = None
        self.ended = False
    ...
    def take_action(self, env):
        while True:
            ...
            if env.is_empty(i, j):
                ...
                print()
                print(
                    'Move is invalid. Type the coordinates (i,j) of an empty cell in the board.')
...
if __name__ == '__main__':
    ...
    print('Trained Agent Wins: {}%;     Random Action Player Wins: {}%;      Draws:{}%'.format(
        agent_wins*100/G, rand_player_wins*100/G, draws*100/G))
