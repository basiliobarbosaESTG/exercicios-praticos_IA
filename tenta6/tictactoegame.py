...
class TicTacToe():
    def __init__(self, player='X', reward_type='goal_reward'):
        ...
        playerX_states = {state for state in states if (
            state.count(0) % 2 == 1 and state.count(1) == state.count(2))}
        playerO_states = {state for state in states if (
            state.count(0) % 2 == 0 and state.count(1) == (state.count(2)+1))}
    ...
    def my_move(self, position):
        ...
        assert self.win(self.me) == False and self.win(
            self.opponent) == False, " Game has already been won"
...
def play_games(n_games, Q_X, Q_O, X_strategy='eps_greedy', O_strategy='eps_greedy', eps_X=0.05, eps_O=0.05, seed=1):
    ...
    for i in tqdm(range(n_games), position=0, leave=True):
        ...
        while True:
            # X plays first
            x_action = t_board_X.pick_best_action(
                Q_X, action_type=X_strategy, eps=eps_X)
            ...
            if t_board_X.is_game_over():  # need to end game here if X makes the winning move
                break
            # O plays second
            o_action = t_board_O.pick_best_action(
                Q_O, action_type=O_strategy, eps=eps_O)
            if first_action_flag == True:
...
#     results_df.columns= ['mu - 2 sd', 'mu + 2 sd']
    ...
    return (win_stats_df), (winning_sequences_X_list, winning_sequences_O_list),\
        (X_first_actions_list, winning_X_first_actions_list), (O_first_actions_list,
                                                               winning_O_first_actions_list)
...
def get_win_seqs(winning_sequences_list):
    ...
    win_seq_df = pd.DataFrame({'winning_sequence': list(
        temp_dict.keys()), 'N': list(temp_dict.values())})
...
def plot_results(win_statistics):
    ...
    win_stats_df_long = pd.melt(
        final_results, var_name='Result', value_name='N')
    ...
    final_results_agg = pd.DataFrame(
        final_results.apply(sum, 0), columns=["N"])
    ...
    _ = sns.heatmap(win_rate_X, annot=True, ax=axs[0, 2]).set_title(
        "Player X: % of wins for first move")
    _ = sns.heatmap(win_rate_O, annot=True, ax=axs[1, 0]).set_title(
        "Player O: % of wins for first move")
...
def train(n_games=1000, alpha=0.5, gamma=0.9, train_X=True, train_O=False, is_random=True, **kwargs):
    ...
    def eps_(flag, i): return 0.05*0.99**i if flag else 1.0
    ...
    for i in tqdm(range(n_games), position=0, leave=True):
        ...
        x_action = t_board_X.pick_best_action(
            Q_X, action_type=X_action_type, eps=eps)
        ...
        while not (t_board_X.is_game_over() or t_board_O.is_game_over()):
            ...
            o_action = t_board_O.pick_best_action(
                Q_O, action_type=O_action_type, eps=eps)
            ...
            if t_board_O.is_game_over():
                ...
                if train_O:
                    # 0 given value of terminal state is 0
                    Q_O[S_O][o_action1d] += alpha * \
                        (R_O + 0 - Q_O[S_O][o_action1d])

                if train_X:
                    # Need to penalize X's previous action if game is over
                    Q_X[S_X][x_action1d] += alpha * \
                        (-R_O + 0 - Q_X[S_X][x_action1d])
            ...
            if train_X:
                x_action_ = t_board_X.pick_best_action(
                    Q_X, action_type='greedy', eps=0.05)  # best action from S_new
                x_action_1d = t_board_X.b2_to_s1[x_action_]
                Q_X[S_X][x_action1d] += alpha * \
                    (R_X + gamma*Q_X[S_X_new]
                     [x_action_1d] - Q_X[S_X][x_action1d])
            ...
            if train_O:
                o_action_ = t_board_O.pick_best_action(
                    Q_O, action_type='greedy', eps=0.05)  # best action from S_new
                o_action_1d = t_board_O.b2_to_s1[o_action_]
                Q_O[S_O][o_action1d] += alpha * \
                    (R_O + gamma*Q_O[S_O_new]
                     [o_action_1d] - Q_O[S_O][o_action1d])
...
Q_X, _, rewards_X, rewards_O = train(
    n_games=5000, alpha=0.5, gamma=0.9, train_X=True, train_O=False, is_random=True)
Q_X_trained = Q_X
...
np.random.seed(1)
# ,_,rewards_X,rewards_O = train(Q_X, Q_O,n_games=8000,alpha = 0.5, gamma = 0.9,train_X=True,train_O=False)
_, Q_O, rewards_X, rewards_O = train(
    n_games=20000, alpha=0.5, gamma=0.5, train_X=False, train_O=True, is_random=True)
Q_O_trained = Q_O
...
plot_results(win_statistics)
np.random.seed(1)
Q_X, Q_O, rewards_X, rewards_O = train(
    n_games=1000, alpha=0.5, gamma=0.9, train_X=True, train_O=False, is_random=False, Q_O=Q_O_trained)
Q_X_retrained = Q_X
